...
class Preprocess():
    ...
    def __init__(self):
        ...
        init_stopward_list = ['i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 'you', "you're", "you've",
...
class Dictionary():
    ...
    def preprocess_all_file(self, pre_path):
        ...
        ## preprocess all document in numerical increasing order, and store Class of Preprocess in preprocess_list
        ...
        for docu_index in range(len(self.preprocess_list)):
            ...
            for term in self.preprocess_list[docu_index].word_dic:
                if first:
                    ...
                    term_nparray = np.append(term_nparray, [[int(self.term_dic[term]), self.tf_idf(term, docu_index)]],
                                             axis=0)
...
if __name__ == '__main__':
    ...
    for cat in range(training_df.shape[1]):
        for item in range(training_df.shape[0]):
            print(int(training_df.iloc[item, cat]) -1)
        print("---")
    # get index of dictionary for training
    ...
    temp = Dictionary()
    temp.preprocess_all_file("D:\Desktop\IR\PA2\IRTM")
